...
def generate_recon_report_for_equity(db):
    ...
    for _, row in fund_name_and_reporting_date.iterrows():
        ...
        print(f"Running EQUITY recon report for fund name: {fund_name}, reporting date: {reporting_date}")
        ...
        if not recon_equity_results.empty:
            print(f"Require EQUITY reconciliation for fund name: {fund_name}, reporting date: {reporting_date}!")
            recon_df = pd.concat([recon_df, recon_equity_results], ignore_index=True)

    print("Completed recon report for EQUITY")

    return recon_df

def generate_recon_report_for_bonds(db):
    fund_name_and_reporting_date = db.fetch_as_df(get_distinct_fund_name_and_reporting_date_query())
    ...
    for _, row in fund_name_and_reporting_date.iterrows():
        fund_name, reporting_date = row['FUND NAME'], row['REPORTING DATE']
        print(f"Running BONDS recon report for fund name: {fund_name}, reporting date: {reporting_date}")
        ...
        if not recon_bonds_results.empty:
            print(f"Require BONDS reconciliation for fund name: {fund_name}, reporting date: {reporting_date}!")
            recon_df = pd.concat([recon_df, recon_bonds_results], ignore_index=True)

    print("Completed recon report for BONDS")
...
def generate_recon_report(db: SQLiteDatabase, output_folder: str) -> pd.DataFrame:
    ...
    for _, row in fund_name_and_reporting_date.iterrows():
        fund_name, reporting_date = row['FUND NAME'], row['REPORTING DATE']
        print(f"Running recon report for fund name: {fund_name}, reporting date: {reporting_date}")
        ...
        if not recon_results.empty:
            print(f"Require reconciliation for fund name: {fund_name}, reporting date: {reporting_date}!")
            recon_df = pd.concat([recon_df, recon_results], ignore_index=True)

    print("Completed recon report")
    ...
    print(f"Saved recon report to {const.recon_report_filename}")
...
def generate_ror_report(db: SQLiteDatabase, output_folder: str) -> pd.DataFrame:
    ...
    for _, row in fund_name_and_reporting_date.iterrows():
        fund_name, reporting_date = row['FUND NAME'], row['REPORTING DATE']
        print(f"Running ROR report for fund name: {fund_name}, reporting date: {reporting_date}")

        ror_query = get_rate_of_return_query(fund_name, reporting_date)
        ror_results = db.fetch_as_df(ror_query)

        ror_df = pd.concat([ror_df, ror_results], ignore_index=True)

    print("Completed ror report")
    ...
    print(f"Saved ROR report to {const.ror_report_filename}")
